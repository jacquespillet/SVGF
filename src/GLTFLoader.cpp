...
void LoadGeometry(tinygltf::Model &GLTFModel, scene *Scene, std::vector<std::vector<uint32_t>> &InstanceMapping)
{
    ...
    for(int MeshIndex=0; MeshIndex<GLTFModel.meshes.size(); MeshIndex++)
    {
        ...
        for(int j=0; j<gltfMesh.primitives.size(); j++)
        {
            ...
            Shape.PositionsTmp.resize(PositionAccessor.count);
            Shape.NormalsTmp.resize(PositionAccessor.count);
            if (TangentIndex>0) Shape.TangentsTmp.resize(PositionAccessor.count);
            if(UVIndex>0) Shape.TexCoordsTmp.resize(PositionAccessor.count);
            for (size_t k = 0; k < PositionAccessor.count; k++)
            {
                glm::vec3 Position;
                {
                    ...
                    Shape.PositionsTmp[k] = glm::vec3(Position.x, Position.y, Position.z);
                }
                ...
                if(NormalIndex>=0)
                {
                    ...
                    Shape.NormalsTmp[k] = glm::vec3(Normal.x, Normal.y, Normal.z);
                }
            ...
            if(IndicesStride == 1)
            {
                ...
                {
                    Shape.IndicesTmp[j].x = Quarter[i+0];
                    Shape.IndicesTmp[j].y = Quarter[i+1];
                    Shape.IndicesTmp[j].z = Quarter[i+2];
                }
            }
            else if(IndicesStride == 2)
            {
                ...
                for(size_t i=0, j=0; i<IndicesAccessor.count; i+=3, j++)
                {
                    Shape.IndicesTmp[j].x = Half[i+0];
                    Shape.IndicesTmp[j].y = Half[i+1];
                    Shape.IndicesTmp[j].z = Half[i+2];
                }
            }
            else
            {
                ...
                for(size_t i=0, j=0; i<IndicesAccessor.count; i+=3, j++)
                {
                    Shape.IndicesTmp[j].x = Uint[i+0];
                    Shape.IndicesTmp[j].y = Uint[i+1];
                    Shape.IndicesTmp[j].z = Uint[i+2];
                }                
            }
            Shape.PreProcess();
        }
